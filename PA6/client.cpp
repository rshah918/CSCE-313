...
RequestChannel* create_new_channel(RequestChannel* mainchan, string ival, int mb, string host, string port){
  ...
  if(ival == "t"){
    cout << host << endl;
    newchan = new TCPRequestChannel(host.c_str(), port.c_str(), RequestChannel::CLIENT_SIDE);
    return newchan;
  }
  char name[1024];
...
int main(int argc, char *argv[])
{
    ...
    string ival = "f"; //choose between FIFO or message queue for IPC
    ...
    int arg;
    while((arg = getopt(argc, argv, "n:p:w:b:m:f:i:h:r:")) != -1){
      switch(arg){
        case 'n':
          n = atoi(optarg);
          break;
...
/*    int pid = fork();
    if (pid == 0){
        ...
        execl ("server", "server", messageFlag.c_str() , "-i", ival.c_str(), (char *)NULL);
    }*/
