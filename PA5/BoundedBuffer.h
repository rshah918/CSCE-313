...
private:
	...
	//for thread safety
	mutex m;
	condition_variable data_available; //wait for pop, signal by push
	condition_variable slot_available; //wait for push, signal for pop
...
public:
	...
	void push(char* data, int len){
		...
		//2. Then push the vector at the end of the queue, wagch out for race condition
		q.push(d);
		l.unlock();
